...
export default function ProjectOverview() {
  ...
  const calculateAvgProcessingTime = (files) => {
    // Calculate realistic processing time based on file count and complexity
    if (files.length === 0) return '0s';
    
    // Base processing time per file (2-8 seconds depending on complexity)
    const baseTime = 3.5;
    const variationFactor = Math.random() * 2 + 1; // 1-3x variation
    const avgTime = baseTime * variationFactor;
